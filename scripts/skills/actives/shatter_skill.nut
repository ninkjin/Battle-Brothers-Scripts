this.shatter_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "击碎";
		this.m.Description = "大弧度挥动武器，以逆时针顺序攻击三个相邻的格子。被击中的目标可能会被打击的力道击退或者趔趄。除非你想减少薪酬支出，否则注意周围的自己人！";
		...
		ret.extend([
			{
				...
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]33%[/color]几率命中时造成趔趄"
			},
			{
				id = 9,
				type = "text",
				icon = "ui/icons/special.png",
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]]33%[/color]几率命中时击退目标"
			},
			{
				id = 6,
				type = "text",
	...
	function applyEffectToTarget( _user, _target, _targetTile )
	{
		...
		else if (applyEffect == 2)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "击退了" + this.Const.UI.getColorizedEntityName(_target));
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "趔趄了" + this.Const.UI.getColorizedEntityName(_target) + "1回合");
			}
		}
	}
