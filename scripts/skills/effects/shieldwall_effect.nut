this.shieldwall_effect <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "盾墙";
		this.m.Description = "在将盾牌举起到了防护姿态的高度后，这个角色获得了更高的防御。";
	...
	function getTooltip()
	{
		...
		return [
			...
			},
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.Math.floor(item.getMeleeDefense() * mult + bonus + proficiencyBonus) + "[/color]近战防御"
			},
			{
				id = 11,
				type = "text",
				icon = "ui/icons/ranged_defense.png",
				text = "[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.Math.floor(item.getRangedDefense() * mult + bonus + proficiencyBonus) + "[/color]远程防御"
			}
		];
	}
