this.helmet <- this.inherit("scripts/items/item", {
	...
	function getTooltip()
	{
		...
		if (this.m.Vision < 0)
		{
			result.push({
				...
				text = "视野[color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.Vision + "[/color]"
			});
		}
		else if (this.m.Vision > 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/vision.png",
				text = "视野[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Vision + "[/color]"
			});
		}
	...
	function onDamageReceived( _damage, _fatalityType, _attacker )
	{
		...
		{
			this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "的" + this.getName() + "被击中造成[b]" + this.Math.floor(_damage) + "[/b]伤害并被摧毁了！");
		...
		{
			this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "的" + this.getName() + "被击中造成[b]" + this.Math.floor(_damage) + "[/b]伤害");
		}
