this.actor <- this.inherit("scripts/entity/tactical/entity", {
	...
	function getTooltip( _targetedWithSkill = null )
	{
		...
		if (this.isHiddenToPlayer())
		{
			tooltip.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]目前不在视野范围内[/color]"
			});
		}
		else
		{
			if (_targetedWithSkill != null && this.isKindOf(_targetedWithSkill, "skill"))
			{
				...
				if (tile.IsVisibleForEntity && _targetedWithSkill.isUsableOn(tile))
				{
					...
					tooltip.push({
						...
						text = "[color=" + this.Const.UI.Color.PositiveValue + "]" + hitchance + "%[/color]命中几率"
					});
				}
			}
			...
			if (this.m.IsActingEachTurn)
			{
				...
				else
				{
					tooltip.push({
						...
						type = "text",
						icon = "ui/icons/initiative.png",
						text = "将在" + turnsToGo + (turnsToGo > 1 ? "个轮次后行动" : "个轮次后行动")
					});
				}
			}
	...
	function onDamageReceived( _attacker, _skill, _hitInfo )
	{
		...
		if (this.m.Hitpoints <= 0)
		{
			...
			if (lorekeeperPotionEffect != null && (!lorekeeperPotionEffect.isSpent() || lorekeeperPotionEffect.getLastFrameUsed() == this.Time.getFrame()))
			{
				...
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "借助博学者的力量重生！");
			}
			else
			{
				...
				if (nineLivesSkill != null && (!nineLivesSkill.isSpent() || nineLivesSkill.getLastFrameUsed() == this.Time.getFrame()))
				{
					...
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "有九命！");
				}
			}
		}
			...
			if (this.m.CurrentProperties.IsAffectedByInjuries && this.m.IsAbleToDie && damage >= this.Const.Combat.InjuryMinDamage && this.m.CurrentProperties.ThresholdToReceiveInjuryMult != 0 && _hitInfo.InjuryThresholdMult != 0 && _hitInfo.Injuries != null)
			{
				...
				while (potentialInjuries.len() != 0)
				{
					...
					if (injury.isValid(this))
					{
						...
						{
							this.worsenMood(this.Const.MoodChange.Injury, "遭受了一处伤势");
						...
						{
							this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的" + this.Const.Strings.BodyPartName[_hitInfo.BodyPart] + "被击中造成[b]" + this.Math.floor(damage) + "[/b]伤害并遭受" + injury.getNameOnly() + "!");
						}
				...
				if (!appliedInjury)
				{
					...
					{
						this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的" + this.Const.Strings.BodyPartName[_hitInfo.BodyPart] + "被击中造成[b]" + this.Math.floor(damage) + "[/b]伤害");
					}
				}
			}
	...
	function onDiscovered()
	{
		if (!this.isPlayerControlled())
		{
			if (this.getFaction() != this.Const.Faction.Player && this.getFaction() != this.Const.Faction.PlayerAnimals)
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this) + "被发现！");
				}
			}
	...
	function kill( _killer = null, _skill = null, _fatalityType = this.Const.FatalityType.None, _silent = false )
	{
		...
		if (!isReallyDead)
		{
			...
			this.logDebug(this.getName() + "失去了意识。");
		}
		else
		{
		...
		if (_killer != null && !_killer.isHiddenToPlayer() && !this.isHiddenToPlayer())
		{
			if (isReallyDead)
			{
				...
				{
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_killer) + "杀死了" + this.Const.UI.getColorizedEntityName(this));
				}
				else
				{
	...
	function retreat()
	{
		if (!this.isHiddenToPlayer())
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this) + "已经撤出了战斗");
		}
