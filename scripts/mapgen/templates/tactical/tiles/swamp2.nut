...
// Source: scripts/mapgen/templates/tactical/tiles/swamp2.nut
// Functions: 3

function init(this)
{
    this.m.Name = "tactical.tile.swamp2";
    this.m.MinX = 1;
    this.m.MinY = 1;
    this.createTileTransition().setBlendIntoSockets(false);
    this.createTileTransition().setBrush(this.Const.Direction.N, "transition_swamp_02_N");
    this.createTileTransition().setBrush(this.Const.Direction.NE, "transition_swamp_02_NE");
    this.createTileTransition().setBrush(this.Const.Direction.SE, "transition_swamp_02_SE");
    this.createTileTransition().setBrush(this.Const.Direction.S, "transition_swamp_02_S");
    this.createTileTransition().setBrush(this.Const.Direction.SW, "transition_swamp_02_SW");
    this.createTileTransition().setBrush(this.Const.Direction.NW, "transition_swamp_02_NW");
    this.createTileTransition().setSocket("socket_earth");
    this.Tactical.setTransitions("tile_swamp_02", this.createTileTransition());
    return;
}
...
function onFirstPass(this, _rect)
{
    ...
    this.Tactical.getTileSquare(_rect.X, _rect.Y).Type = this.Const.Tactical.TerrainType.RoughGround;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).Subtype = this.Const.Tactical.TerrainSubtype.MoistEarth;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).BlendPriority = this.Const.Tactical.TileBlendPriority.Swamp2;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).IsBadTerrain = false;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).setBrush("tile_swamp_02");
