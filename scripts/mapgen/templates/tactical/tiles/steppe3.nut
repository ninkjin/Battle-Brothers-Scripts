...
// Source: scripts/mapgen/templates/tactical/tiles/steppe3.nut
// Functions: 2
...
function init(this)
{
    ...
    this.createTileTransition().setBrush(this.Const.Direction.SE, "transition_sand_SE");
    this.createTileTransition().setBrush(this.Const.Direction.S, "transition_sand_S");
    this.createTileTransition().setBrush(this.Const.Direction.SW, "transition_sand_SW");
    this.createTileTransition().setSocket("socket_steppe");
    this.Tactical.setTransitions("tile_steppe_03", this.createTileTransition());
    return;
}
...
function onFirstPass(this, _rect)
{
    ...
    this.Tactical.getTileSquare(_rect.X, _rect.Y).Subtype = this.Const.Tactical.TerrainSubtype.DrySteppe;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).BlendPriority = this.Const.Tactical.TileBlendPriority.Steppe3;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).IsBadTerrain = false;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).setBrush("tile_steppe_03");
