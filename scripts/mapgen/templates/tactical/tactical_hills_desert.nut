...
// Source: scripts/mapgen/templates/tactical/tactical_hills_desert.nut
// Functions: 3
...
function fill(this, _rect, _properties, _pass)
{
    ...
    if ((0 < 3000) && (0 < 3000))
    {
        ...
        this.MapGen.get("tactical.patch.hill_desert").fill({}, _properties);
    }
    if (6 != 0)
    {
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            {
                this.Tactical.getTileSquare(_rect.X, _rect.Y).Level = 1;
                this.MapGen.get("tactical.tile.desert5").fill({}, _properties);
            }
            if (this.Math.rand(1, 100) < 50)
            {
                this.MapGen.get("tactical.tile.desert1").fill({}, _properties);
            }
            if (this.Math.rand(1, 100) < 90)
            {
                this.MapGen.get("tactical.tile.desert2").fill({}, _properties);
            }
            this.MapGen.get("tactical.tile.desert7").fill({}, _properties);
        }
    }
