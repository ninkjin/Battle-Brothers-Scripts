this.acid_effect <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]20%[/color]头部盔甲的每回合损失"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]20%[/color]身体盔甲的每回合损失"
			});
		}
	...
	function onAdded()
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "的身体被喷溅了酸液");
		}
	}
