...
// Source: scripts/mapgen/templates/tactical/tactical_hills_snow.nut
// Functions: 3
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            else
            {
                ...
                if ((this.Math <= (40 + ((7 - this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y))) * 7))) && (this.Math <= (40 + ((7 - this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2)) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y))) * 7)))))
                {
                    return ((this.Math <= (40 + ((7 - this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2)) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y))) * 7))) && (this.Math <= (40 + ((7 - this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2)) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y))) * 7))));
                    this.Tactical.getTileSquare(_rect.X, _rect.Y).Type = 0;
                    this.MapGen.get("tactical.tile.snow3").fill({}, null);
                }
            }
        }
    }
...
function fill(this, _rect, _properties, _pass)
{
    ...
    [].push(this.MapGen.get("tactical.patch.snow"));
    [].push(this.MapGen.get("tactical.patch.snow"));
    [].push(this.MapGen.get("tactical.patch.snow"));
    [].push(this.MapGen.get("tactical.patch.snow"));
    [].push(this.MapGen.get("tactical.patch.snow"));
    [].push(this.MapGen.get("tactical.patch.snow_less"));
    if ((0 < 3000) && (0 < 3000))
    {
        ...
        this.MapGen.get("tactical.patch.hill_snow").fill({}, _properties);
    }
    if (6 != 0)
    {
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            if (this.Math.rand(1, 100) > (100 - ((((0 + 18) + 18) + 18) * 10)))
            {
                this.Tactical.getTileSquare(_rect.X, _rect.Y).Level = 1;
            }
            if (this.Math.rand(1, 100) < 60)
            {
                this.MapGen.get("tactical.tile.snow1").fill({}, _properties);
            }
            if (this.Math.rand(1, 100) < 80)
            {
                this.MapGen.get("tactical.tile.snow2").fill({}, _properties);
            }
            this.MapGen.get("tactical.tile.snow3").fill({}, _properties);
        }
    }
