this.lunge_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "突刺";
		this.m.Description = "出其不意的快速冲向2格外的目标，然后对它进行精确的刺击。你的速度越快，造成的伤害就越大。";
		this.m.KilledString = "切片";
	...
	function getTooltip()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsRooted)
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]无法在定身时使用[/color]"
			});
		}
	...
	function onTeleportDone( _entity, _tag )
	{
		...
		foreach( actor in ZOC )
		{
			...
			if (actor.onAttackOfOpportunity(_entity, true))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "突刺但被击退");
				}
