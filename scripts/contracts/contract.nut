this.contract <- {
	...
	function getScreen( _id )
	{
		...
		this.logError("屏幕 \"" + _id + "\" not found for contract \"" + this.m.Type + "\".");
		return null;
	}
	...
	function getState( _id )
	{
		...
		this.logError("国家\"" + _id + "\" not found for contract \"" + this.m.Type + "\".");
		return null;
	}
	...
	function cancel()
	{
		...
		if (this.m.Faction != 0)
		{
			this.World.FactionManager.getFaction(this.m.Faction).addPlayerRelation(this.Const.World.Assets.RelationContractCancel, "撕毁了合同");

			if (this.m.Payment.Advance != 0)
			{
				this.World.FactionManager.getFaction(this.m.Faction).addPlayerRelation(this.Const.World.Assets.RelationContractCancelAdvance, "撕毁了合同");
			}
		}
	...
	function getTileToSpawnLocation( _pivot, _minDist, _maxDist, _notOnTerrain = [], _allowRoad = true, _needsLandConnection = true, _needsLandConnectionToPlayer = false )
	{
		...
		while (1)
		{
			...
			if (_needsLandConnection)
			{
				...
				if (path.getSize() > _maxDist * pathDistanceMult)
				{
					continue;
				}
			}
	...
	function spawnEnemyPartyAtBase( _factionType, _resources )
	{
		...
		if (_factionType == this.Const.FactionType.Bandits)
		{
			...
			party.setDescription("一伙为了食物外出狩猎的强壮强盗。");
