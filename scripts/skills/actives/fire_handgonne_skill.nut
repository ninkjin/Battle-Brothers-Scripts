	...
	function getTooltip()
	{
		...
		if (10 + this.m.AdditionalAccuracy >= 0)
		{
			ret.push({
				...
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]+" + (10 + this.m.AdditionalAccuracy) + "%[/color]额外命中，同时每格距离有[color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color]命中惩罚，障碍物或角色的阻挡不会影响这个几率。"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有[color=" + this.Const.UI.Color.NegativeValue + "]" + (10 + this.m.AdditionalAccuracy) + "%[/color]额外命中，同时每格距离有[color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color]命中惩罚，障碍物或角色的阻挡不会影响这个几率。"
			});
		}
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color]弹药剩余"
			});
		}
		else
		{
