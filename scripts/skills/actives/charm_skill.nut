	...
	function onDelayedEffect( _tag )
	{
		...
		this.Time.scheduleEvent(this.TimeUnit.Virtual, time, function ( _e )
		{
			...
			if (target.checkMorale(0, -35 + bonus, this.Const.MoraleCheckType.MentalAttack))
			{
				...
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + "依靠他的决心抵抗了魅惑");
				}
			...
			if (target.getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
			{
				if (!_user.isHiddenToPlayer() && !target.isHiddenToPlayer())
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + "非自然的生理功能帮助他抵抗了魅惑");
				}
			...
			if (!_user.isHiddenToPlayer() && !target.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(target) + "被魅惑了");
			}
