this.lindwurm_acid_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "这个角色的盔甲被喷溅了林德虫的酸性血液，还会被继续腐蚀[color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color]回合。";
	}
	...
	function getTooltip()
	{
		...
		if (!this.getContainer().getActor().getFlags().has("head_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]10%[/color]头部盔甲每回合都会损失"
			});
		}
		...
		if (!this.getContainer().getActor().getFlags().has("body_immune_to_acid"))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]10%[/color]身体盔甲每回合都会损失"
			});
		}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "非自然的生理功能帮助他免受酸液影响");
			}
