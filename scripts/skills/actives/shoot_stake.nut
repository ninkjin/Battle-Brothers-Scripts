	...
	function getTooltip()
	{
		...
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有[color=" + this.Const.UI.Color.NegativeValue + "]" + (10 + this.m.AdditionalAccuracy) + "%[/color]额外命中，同时每格距离有[color=" + this.Const.UI.Color.NegativeValue + "]" + (-3 + this.m.AdditionalHitChance) + "%[/color]命中惩罚"
			});
		}
		...
		else
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]需要装备非空弩矢袋[/color]"
			});
		}
