this.item_container <- {
	...
	function addToBag( _item, _slot = -1 )
	{
		...
		{
			this.logWarning("尝试添加物品" + _item.getName() + "到背包, 但已经被放在其他地方了");
			return false;
		}
		...
		if (_slot != -1)
		{
			...
			{
				this.logWarning("尝试添加物品" + _item.getName() + "到背包栏位，但其不是空的或者被锁定");
				return false;
			}
		...
		{
			this.logWarning("无法添加" + _item.getName() + "到背包，因为没有发现空槽位");
			return false;
		}
	}
	...
	function removeFromBag( _item )
	{
		...
		{
			this.logWarning("尝试从背包中移除物品" + _item.getName() + "，但被放置在其他地方");
			return false;
		}
	...
	function removeFromBagSlot( _slot )
	{
		...
		if (item == null)
		{
			this.logWarning("尝试从空的背包栏位中移除物品");
			return false;
		}
	...
	function equip( _item )
	{
		...
		if (_item.getCurrentSlotType() != this.Const.ItemSlot.None)
		{
			this.logWarning("尝试装备物品。" + _item.getName() + "，但它已被放置在其他地方");
			return false;
		}
	...
	function unequip( _item )
	{
		...
		{
			this.logWarning("尝试卸下物品" + _item.getName() + "，但其并未被装备");
			return false;
		}
	...
	function swap( _itemA, _itemB )
	{
		...
		{
			this.logWarning("无法换位，物品不使用相同栏位！");
			return false;
		}
		...
		{
			this.logWarning("无法换位，两个物品均未被装备！");
			return false;
		}
	...
	{
		this.logInfo("##############################################");
		this.logInfo("# 展示物品栏于" + this.getActor().getName());
