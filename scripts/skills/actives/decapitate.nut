this.decapitate <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "斩首";
		this.m.Description = "力求当即斩首目标的毁灭性打击。目标的伤势越严重，造成的生命值伤害越高。如果可能，总是斩首被杀死的目标。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "造成[color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_min + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_max * 2 + "[/color]造成伤害取决于目标已受伤的程度，其中[color=" + this.Const.UI.Color.DamageValue + "]0[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_direct_max + "[/color]可无视盔甲"
		});
