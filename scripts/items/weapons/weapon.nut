	...
	function getTooltip()
	{
		...
		if (this.m.RegularDamage > 0)
		{
			result.push({
				...
				text = "伤害[color=" + this.Const.UI.Color.DamageValue + "]" + this.m.RegularDamage + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + this.m.RegularDamageMax + "[/color]"
			});
		}
		...
		if (this.m.DirectDamageMult > 0)
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.DamageValue + "]" + this.Math.floor((this.m.DirectDamageMult + this.m.DirectDamageAdd) * 100) + "%[/color]穿甲伤害"
			});
		}
		...
		if (this.m.ArmorDamageMult > 0)
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.DamageValue + "]" + this.Math.floor(this.m.ArmorDamageMult * 100) + "%[/color]破甲效率"
			});
		}
		...
		if (this.m.ChanceToHitHead > 0)
		{
			result.push({
				...
				text = "击中头部几率[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.ChanceToHitHead + "%[/color]"
			});
		}
		...
		if (this.m.AdditionalAccuracy > 0)
		{
			result.push({
				...
				text = "有额外[color=" + this.Const.UI.Color.PositiveValue + "]+" + this.m.AdditionalAccuracy + "%[/color]命中几率"
			});
		}
		else if (this.m.AdditionalAccuracy < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有额外[color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.AdditionalAccuracy + "%[/color]命中几率"
			});
		}
		...
		if (this.m.RangeMax > 1)
		{
			result.push({
				...
				text = "攻击范围为[color=" + this.Const.UI.Color.PositiveValue + "]" + this.getRangeMax() + "[/color]格"
			});
		}
		...
		if (this.m.FatigueOnSkillUse > 0 && (this.getContainer() == null || !this.getContainer().getActor().getCurrentProperties().IsProficientWithHeavyWeapons))
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "武器技能[color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.FatigueOnSkillUse + "[/color]额外疲劳积累"
			});
		}
		else if (this.m.FatigueOnSkillUse < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "武器技能[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.FatigueOnSkillUse + "[/color]疲劳积累"
			});
		}
		...
		if (this.m.AmmoMax > 0)
		{
			if (this.m.Ammo != 0)
			{
				result.push({
					...
					text = "包含[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Ammo + "[/color]发弹药"
				});
			}
			else
			{
				result.push({
					...
					text = "[color=" + this.Const.UI.Color.NegativeValue + "]空了并且无用[/color]"
				});
			}
		}
