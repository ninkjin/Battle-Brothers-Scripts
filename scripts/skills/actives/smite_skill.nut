this.smite_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "猛击";
		this.m.Description = "一种缓慢的过顶击，用全部力量把目标砸成碎片。任何被击中的目标都会趔趄并丧失主动性。";
	...
	function onUse( _user, _targetTile )
	{
		...
		if (success && _targetTile.IsOccupiedByActor && !_targetTile.getEntity().isNonCombatant())
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "趔趄了" + this.Const.UI.getColorizedEntityName(_targetTile.getEntity()) + "1回合");
			}
		}
