this.tactical_state <- this.inherit("scripts/states/state", {
	...
	function setActionStateBySkillIndex( _skillIndex )
	{
		...
		{
			this.logDebug("TurnSequenceBar::setActionStateBySkillId(无活动单位 | 敌人)");
			return;
		}
		...
		if (_skillIndex >= 0 && _skillIndex < activeSkills.len())
		{
			...
			if (skill == null)
			{
				this.logDebug("TurnSequenceBar::setActionStateBySkillIndex(未选择技能)");
				return;
			}
	...
	function setActionStateBySkillId( _skillId )
	{
		...
		if (skill == null)
		{
			this.logDebug("TurnSequenceBar::setActionStateBySkillId(未选择技能)");
			return;
		}
		...
		if (skill != null && skill.isUsable() && skill.isAffordable())
		{
			...
			if (skill.isUsableOn(_targetTile))
			{
				if (!_targetTile.IsEmpty)
				{
					...
					{
						this.logDebug("[" + _activeEntity.getName() + "]执行技能 [" + skill.getName() + "] 于目标 [" + targetEntity.getName() + "]");
					}
				}
			...
			if (!this.isScenarioMode())
			{
				...
				foreach( bro in playerRoster )
				{
					...
					else if (this.m.StrategicProperties.IsUsingSetPlayers && bro.isPlacedOnMap())
					{
						...
						{
							bro.improveMood(this.Const.MoodChange.BattleWon, "赢了一场战斗");
						}
					}
			...
			if (!this.isScenarioMode())
			{
				...
				foreach( bro in playerRoster )
				{
					...
					else if (bro.getPlaceInFormation() <= 17)
					{
						...
						{
							bro.worsenMood(this.Const.MoodChange.BattleLost, "输了一场战斗");
						}
	...
	function showFleeScreen( _tag = null )
	{
		...
		this.Tooltip.hide();
		this.m.TacticalScreen.hide();
		this.m.TacticalDialogScreen.show("从战斗中撤退", "你确定吗？", this.Const.Strings.UI.FleeDialogueConsequences, "撤退！", "取消", this.tactical_flee_screen_onFleePressed.bindenv(this), this.tactical_flee_screen_onCancelPressed.bindenv(this));
		this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
		this.m.MenuStack.push(function ()
		{
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
	...
	function showRetreatScreen( _tag = null )
	{
		...
		this.Tooltip.hide();
		this.m.TacticalScreen.hide();
		this.m.TacticalDialogScreen.show("敌人撤退了", "", this.Const.Strings.UI.RetreatDialogueConsequences, "结束了", "赶尽杀绝！", this.tactical_retreat_screen_onYesPressed.bindenv(this), this.tactical_retreat_screen_onNoPressed.bindenv(this));
		this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
		this.m.MenuStack.push(function ()
		{
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
	...
	function toggleMenuScreen()
	{
		...
		if (!hasBacksteps)
		{
			...
			this.Tooltip.hide();
			this.m.TacticalScreen.hide();
			this.m.TacticalMenuScreen.show(allowRetreat, allowQuit, !this.isScenarioMode() && this.World.Assets.isIronman() ? "退出并退休" : "退出");
			this.Cursor.setCursor(this.Const.UI.Cursor.Hand);
			this.m.MenuStack.push(function ()
			{
				this.m.TacticalMenuScreen.hide();
