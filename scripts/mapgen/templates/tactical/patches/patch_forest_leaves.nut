...
// Source: scripts/mapgen/templates/tactical/patches/patch_forest_leaves.nut
// Functions: 2
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            else
            {
                ...
                this.MapGen.get("tactical.tile.moss2").fill({}, _properties, 1);
            }
        }
    }
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            else
            {
                ...
                this.MapGen.get("tactical.tile.moss2").fill({}, _properties, 2);
            }
        }
    }
    return;
...
function init(this)
{
    this.m.Name = "tactical.patch.forest_leaves";
    this.m.MinX = 10;
    this.m.MinY = 10;
    return;
}
