this.knock_back <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "击退";
		this.m.Description = "使用盾牌击退目标一格。被击中的目标会积累疲劳，并且如果被推下多层高度还会受到伤害。成功的击退将取消目标的盾墙，矛墙，反击技能，定身的目标不能被击退。";
	...
	function getTooltip()
	{
		...
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有[color=" + this.Const.UI.Color.PositiveValue + "]+25%[/color]命中增益"
			});
		}
