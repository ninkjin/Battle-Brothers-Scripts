this.large_powder_bag <- this.inherit("scripts/items/ammo/ammo", {
	...
	function create()
	{
		...
		this.m.Name = "大火药包";
		this.m.Description = "一大包黑火药，用来装填奇异的火器。如果你有足够的弹药，每次战斗后都会自动补充。";
	...
	function getTooltip()
	{
		...
		if (this.m.Ammo != 0)
		{
			result.push({
				...
				text = "含有可发射[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.Ammo + "[/color]次的火药"
			});
		}
		else
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]空的无用[/color]"
			});
		}
