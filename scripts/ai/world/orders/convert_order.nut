...
// Source: scripts/ai/world/orders/convert_order.nut
// Functions: 8
            ...
            foreach (local key, value in r115)
            {
                if ((null == this.m.TargetID) && (null == this.m.TargetID))
                {
                    ...
                    if ((!this.World.Assets.isPermanentDestruction) && (!this.World.Assets.isPermanentDestruction))
                    {
                        ...
                        this.World.Statistics.addNews("crisis_undead_town_destroyed", this.World.Statistics.createNews());
                        this.World.FactionManager.addGreaterEvilStrength(this.Const.Factions.GreaterEvilStrengthOnTownDestroyed);
                        null.setActive(false, false);
                        null.fadeOutAndDie();
                        this.World.EntityManager.updateSettlementHeat();
                        this.World.spawnLocation("scripts/entity/world/locations/undead_necropolis_location", null.getTile().Coords).setName(null.getName());
                        this.World.spawnLocation("scripts/entity/world/locations/undead_necropolis_location", null.getTile().Coords).setSprite(null.m.Sprite);
                        this.World.spawnLocation("scripts/entity/world/locations/undead_necropolis_location", null.getTile().Coords).onSpawned();
                        this.World.spawnLocation("scripts/entity/world/locations/undead_necropolis_location", null.getTile().Coords).setBanner(_entity.getBanner());
                        this.World.FactionManager.getFaction(_entity.getFaction()).addSettlement(this.World.spawnLocation("scripts/entity/world/locations/undead_necropolis_location", null.getTile().Coords), false);
                    }
