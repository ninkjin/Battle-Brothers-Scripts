...
// Source: scripts/mapgen/templates/tactical/tactical_forest.nut
// Functions: 3
...
function fill(this, _rect, _properties, _pass)
{
    ...
    [].push(this.MapGen.get("tactical.patch.forest"));
    [].push(this.MapGen.get("tactical.patch.forest"));
    [].push(this.MapGen.get("tactical.patch.forest"));
    [].push(this.MapGen.get("tactical.patch.forest"));
    [].push(this.MapGen.get("tactical.patch.forest"));
    if ((0 < 5000) && (0 < 5000))
    {
        ...
        this.MapGen.get("tactical.patch.forest_thick").fill({}, _properties);
    }
    if (20 != 0)
    {
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            this.MapGen.get("tactical.tile.moss1").fill({}, _properties);
        }
    }
