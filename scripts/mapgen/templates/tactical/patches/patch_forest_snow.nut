...
// Source: scripts/mapgen/templates/tactical/patches/patch_forest_snow.nut
// Functions: 2
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            else
            {
                ...
                {
                    this.MapGen.get("tactical.tile.snow3").fill({}, _properties, 1);
                }
                this.MapGen.get("tactical.tile.snow1").fill({}, _properties, 1);
            }
        }
    }
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            ...
            else
            {
                ...
                this.MapGen.get("tactical.tile.snow1").fill({}, _properties, 2);
            }
        }
    }
    return;
