this.tactical_entity_manager <- {
	...
	function onResurrect( _info, _force = false )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(entity) + "从死亡中爬起");
		}
		...
		if (this.World.Assets.getOrigin().getID() == "scenario.manhunters")
		{
			...
			if (slaves <= nonSlaves)
			{
				foreach( bro in roster )
				{
					...
					if (bro.getBackground().getID() != "background.slave")
					{
						bro.worsenMood(this.Const.MoodChange.TooFewSlavesInBattle, "参战的负债者太少");
					}
				}
			}
		}
