this.defend_nomad_action <- this.inherit("scripts/factions/faction_action", {
	m = {
		Settlement = null
	},
	function create()
	{
		this.m.ID = "defend_nomad_action";
	...
	function onExecute( _faction )
	{
		...
		{
			local party = this.getFaction().spawnEntity(spawnpoints[i], "Nomads", false, this.Const.World.Spawn.NomadDefenders, this.m.Settlement.getResources() * 0.66);
