...
// Source: scripts/mapgen/templates/tactical/tactical_forest_snow.nut
// Functions: 3
    ...
    if (_rect.X < (_rect.X + _rect.W))
    {
        if (_rect.Y < (_rect.Y + _rect.H))
        {
            if ((this.Math.rand(1, 100) <= 20) && (this.Math.rand(1, 100) <= 20))
            {
                ...
                this.Tactical.getTileSquare(_rect.X, _rect.Y).spawnObject("entity/tactical/objects/tree_stump3");
            }
            if (this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y)) <= ((this.Const.Tactical.Settings.CampRadius + _properties.AdditionalRadius) + 3)))
            {
                ...
                if (this.Math.rand(1, 100) <= (40 + ((7 - this.Tactical.getTileSquare(((_rect.X + (_rect.W / 2) + _properties.ShiftX), ((_rect.Y + (_rect.H / 2)) + _properties.ShiftY)).getDistanceTo(this.Tactical.getTileSquare(_rect.X, _rect.Y))) * 7))))
                {
...
function fill(this, _rect, _properties, _pass)
{
    ...
    [].push(this.MapGen.get("tactical.patch.forest_snow"));
    [].push(this.MapGen.get("tactical.patch.forest_snow"));
    [].push(this.MapGen.get("tactical.patch.forest_snow"));
    [].push(this.MapGen.get("tactical.patch.forest_snow"));
    [].push(this.MapGen.get("tactical.patch.forest_snow"));
    if ((0 < 5000) && (0 < 5000))
    {
        ...
        this.MapGen.get("tactical.patch.forest_thick_snow").fill({}, _properties);
    }
    if (20 != 0)
    {
