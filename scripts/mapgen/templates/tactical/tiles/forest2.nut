...
// Source: scripts/mapgen/templates/tactical/tiles/forest2.nut
// Functions: 5
...
function fillWithMushrooms(this, _rect, _properties, _pass)
{
    ...
    this.Tactical.getTileSquare(_rect.X, _rect.Y).setBrush("tile_forest_02");
...
function init(this)
{
    ...
    this.createTileTransition().setBrush(this.Const.Direction.N, "transition_forest_02_N");
    this.createTileTransition().setBrush(this.Const.Direction.NE, "transition_forest_02_NE");
    this.createTileTransition().setBrush(this.Const.Direction.SE, "transition_forest_02_SE");
    this.createTileTransition().setBrush(this.Const.Direction.S, "transition_forest_02_S");
    this.createTileTransition().setBrush(this.Const.Direction.SW, "transition_forest_02_SW");
    this.createTileTransition().setBrush(this.Const.Direction.NW, "transition_forest_02_NW");
    this.createTileTransition().setSocket("socket_earth");
    this.Tactical.setTransitions("tile_forest_02", this.createTileTransition());
    return;
}
...
function onFirstPass(this, _rect, _objectSpawnChanceMult)
{
    ...
    if ((this.Math < (this.m.ChanceToSpawnObject * _objectSpawnChanceMult) && (this.Math < (this.m.ChanceToSpawnObject * _objectSpawnChanceMult))))
    {
        ...
        {
            this.Tactical.getTileSquare(_rect.X, _rect.Y).spawnObject(this.m.ObjectsTreesSmall["this.Math.rand(0, (this.m.ObjectsTreesSmall.len() - 1))"]);
        ...
        {
            if (_rect.IsTreeAllowed)
            {
                this.Tactical.getTileSquare(_rect.X, _rect.Y).spawnObject(this.m.ObjectsTrees["this.Math.rand(0, (this.m.ObjectsTrees.len() - 1))"]);
            }
            else
            {
                ...
                if (this.Math.rand(1, 100) <= 80)
                {
                    ...
                    {
                        this.Tactical.getTileSquare(_rect.X, _rect.Y).spawnDetail("hiding_shrubbery_01_front");
                    }
                    this.Tactical.getTileSquare(_rect.X, _rect.Y).spawnDetail(("forest_roots_0" + this.Math.rand(1, 3)));
                }
            }
        }
    ...
    else
    {
        ...
        if (this.Math.rand(1, 100) < (this.m.ChanceToSpawnHiding + ((((0 + 5) + 5) + 5) * this.m.ChanceToSpawnHidingVicinityBonus)))
        {
