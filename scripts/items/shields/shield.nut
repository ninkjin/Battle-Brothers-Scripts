	...
	function getTooltip()
	{
		...
		if (this.m.FatigueOnSkillUse > 0 && (this.getContainer() == null || !this.getContainer().getActor().getCurrentProperties().IsProficientWithHeavyWeapons))
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "盾牌技能[color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.FatigueOnSkillUse + "[/color]额外疲劳积累"
			});
		}
		else if (this.m.FatigueOnSkillUse < 0)
		{
			result.push({
				...
				type = "text",
				icon = "ui/icons/fatigue.png",
				text = "盾牌技能[color=" + this.Const.UI.Color.PositiveValue + "]" + this.m.FatigueOnSkillUse + "[/color]疲劳积累"
			});
		}
		...
		if (this.m.Condition == 0)
		{
			result.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]破损不能使用[/color]"
			});
		}
