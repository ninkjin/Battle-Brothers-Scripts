...
// Source: scripts/mapgen/templates/tactical/tiles/desert2.nut
// Functions: 3
...
function init(this)
{
    ...
    this.Tactical.setTransitions("tile_desert_02", this.createTileTransition());
    return;
}
...
function onFirstPass(this, _rect)
{
    ...
    this.Tactical.getTileSquare(_rect.X, _rect.Y).BlendPriority = this.Const.Tactical.TileBlendPriority.Desert2;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).IsBadTerrain = false;
    this.Tactical.getTileSquare(_rect.X, _rect.Y).setBrush("tile_desert_02");
    if (_rect.IsEmpty)
    {
        return;
    }
