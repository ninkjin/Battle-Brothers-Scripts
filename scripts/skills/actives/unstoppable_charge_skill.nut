this.unstoppable_charge_skill <- this.inherit("scripts/skills/skill", {
	...
	function applyEffectToTarget( _user, _target, _targetTile )
	{
		...
		if (applyEffect == 1 && !_target.getCurrentProperties().IsImmuneToStun)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + "击晕了" + this.Const.UI.getColorizedEntityName(_target) + "1回合");
			}
		}
	...
	function onTeleportDone( _entity, _tag )
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_entity) + "冲锋");
		}
