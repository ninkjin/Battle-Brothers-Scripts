this.gash_skill <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "致伤";
		this.m.Description = "一种瞄准要害划砍攻击，更可能造成严重的伤势。";
	...
	function getTooltip()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsSpecializedInSwords)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有降低[color=" + this.Const.UI.Color.NegativeValue + "]50%[/color]的致伤阈值"
			});
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有降低[color=" + this.Const.UI.Color.NegativeValue + "]33%[/color]的致伤阈值"
			});
		}
