...
// Source: scripts/ai/tactical/behaviors/ai_attack_decapitate.nut
// Functions: 4
...
function getBestTarget(this, _entity, _skill, _targets)
{
    foreach (local key, value in r59)
    {
        ...
        {
            if (null.getHitpointsPct() > 0.800000011920929)
            {
            }
            if (null.getHitpoints() <= _entity.getCurrentProperties().getRegularDamageAverage())
            {
            }
            if ((100 >= 100) && (100 >= 100))
            {
            ...
            if (this.queryTargetValue(_entity, null, _skill) > 0.0)
            {
            }
            return _entity;
        }
