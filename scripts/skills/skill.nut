this.skill <- {
	...
	function getDefaultTooltip()
	{
		...
		if (this.m.DirectDamageMult == 1.0)
		{
			ret.push({
				...
				text = "造成[color=" + this.Const.UI.Color.DamageValue + "]" + damage_direct_min + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_direct_max + "[/color]无视护甲的伤害"
			});
		}
		...
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/regular_damage.png",
				text = "造成[color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_min + "[/color] - [color=" + this.Const.UI.Color.DamageValue + "]" + damage_regular_max + "生命值伤害[/color]"
			});
		}
		...
		if (this.m.Container.getActor().getSkills().hasSkill("trait.oath_of_honor") && (this.m.IsWeaponSkill && this.m.IsRanged || this.m.IsOffensiveToolSkill))
		{
			ret.push({
				...
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]无法使用，因为这个角色已经宣誓拒绝使用远程武器或工具[/color]"
			});
		}
	...
	{
		return "[i]消耗" + (this.isAffordableBasedOnAPPreview() ? "[b][color=" + this.Const.UI.Color.PositiveValue + "]" + this.getActionPointCost() : "[b][color=" + this.Const.UI.Color.NegativeValue + "]" + this.getActionPointCost()) + "AP[/color][/b]并产生" + (this.isAffordableBasedOnFatiguePreview() ? "[b][color=" + this.Const.UI.Color.PositiveValue + "]" + this.getFatigueCost() : "[b][color=" + this.Const.UI.Color.NegativeValue + "]" + this.getFatigueCost()) + "疲劳[/color][/b][/i]\n";
	}
	...
	function use( _targetTile, _forFree = false )
	{
		...
		if (!_forFree)
		{
			this.logDebug(user.getName() + " 使用技能 " + this.getName());
		}
	...
	function getHitFactors( _targetTile )
	{
		...
		if (this.m.IsRanged && myTile.getDistanceTo(_targetTile) > 1)
		{
			if (_targetTile.IsOccupiedByActor)
			{
				ret.push({
					icon = "ui/tooltips/negative.png",
					text = "距离" + _targetTile.getDistanceTo(user.getTile())
				});
			}
	...
	function attackEntity( _user, _targetEntity, _allowDiversion = true )
	{
		...
		if (!_user.isHiddenToPlayer() && !_targetEntity.isHiddenToPlayer())
		{
			...
			else if (this.isUsingHitchance())
			{
				if (isHit)
				{
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + "使用" + this.getName() + "并命中了" + this.Const.UI.getColorizedEntityName(_targetEntity) + "(几率:" + this.Math.min(95, this.Math.max(5, toHit)) + "，掷出:" + rolled + ")");
				}
				else
				{
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + "使用" + this.getName() + "但未命中" + this.Const.UI.getColorizedEntityName(_targetEntity) + "(几率:" + this.Math.min(95, this.Math.max(5, toHit)) + "，掷出:" + rolled + ")");
				}
			}
			else
			{
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + "使用" + this.getName() + "并命中了" + this.Const.UI.getColorizedEntityName(_targetEntity));
			}
		}
	...
	function onShieldHit( _info )
	{
		...
		if (shield.getCondition() == 0)
		{
			...
			{
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(user) + "摧毁了" + this.Const.UI.getColorizedEntityName(targetEntity) + "的盾牌");
			}
		}
		else
		{
			if (!user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(user) + "击中了" + this.Const.UI.getColorizedEntityName(targetEntity) + "的盾牌并造成1点伤害");
			}
